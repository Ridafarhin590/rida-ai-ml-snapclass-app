...
def send_note(sender_id, receiver_id, content):
    data = {
        "sender_id": sender_id,
        "receiver_id": receiver_id,  # None = all
        "content": content
    }
...
def is_student_allowing_messages(student_id):
    response = supabase.table("students").select("allow_messages").eq("student_id", student_id).execute()
    return response.data[0]["allow_messages"] if response.data else False
...
def send_student_message(sender_id, receiver_id, message):
    ...
    return supabase.table("messages").insert(data).execute().data
